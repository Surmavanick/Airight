...
- `OPENAI_API_KEY` (optional): server-only credential for the AI-assisted **Aright plan**. It drafts detailed Human-work steps, acceptance criteria, evidence suggestions and advisory task reviews; it is not a detector and never changes the original model score or task completion.
- `OPENAI_MODEL` (optional): defaults to `gpt-5.6-luna`, the cost-sensitive Responses API model used for bounded structured output.
- `OPENAI_TIMEOUT_MS` (optional): defaults to 75000 so a multi-task detailed plan can finish without exceeding the Vercel function budget.
...
Node 18+ and Python 3.12 are required. The first setup downloads roughly 1 GB of model data into the normal Hugging Face cache. Model weights load lazily and remain in one persistent worker process while the server is running. To enable the external image check, copy `.env.example` to `.env`, set `AIORNOT_API_KEY`, optionally set the independently issued `AIORNOT_API_KEY_BACKUP`, and restart Aright. Set `OPENAI_API_KEY` in the same ignored file to enable the AI-assisted Aright plan; the key is never sent to the browser or Python worker.
...
Every modality also receives a separate **51% Human contribution plan**. For code, Aright calculates concrete substantive lines, core files and behavior tests to hand-write; text uses words, images use traceable edit categories, video uses screened shots, and audio uses recorded/edited seconds. Selecting a checklist box only stages the task; a separate **Confirm done** action is required before it counts toward workflow progress. Even confirmed work is a user declaration: it never changes the original detector signal or proves authorship.

When `OPENAI_API_KEY` is configured, the **Aright plan** is generated automatically after analysis and when an older record without a current usable plan is opened. It enriches every deterministic checklist task with bounded, structured steps, acceptance criteria and evidence suggestions, and prepares an editable evidence statement from facts already present in the report. The interface uses Aright branding while transparently disclosing that this drafting is AI-assisted using OpenAI. `Re-check evidence` reviews a task's declared state, bounded attachment metadata and the editable statement as supported, partial or unsupported; it never completes the task, changes the detector score or turns a draft into proof. Attachment bytes remain in IndexedDB and are not sent to OpenAI. Requests use the Responses API with `store: false`; provider processing and the OpenAI API data policy still apply.

Every evidence export records the model ID, revision, raw model response, score semantics, SHA-256 asset fingerprint where available, and the action-plan state. Each checklist task can also hold optional supporting files such as licences, approvals, source files, or before/after exports. File bytes stay in that browser's IndexedDB; `localStorage` and the standalone JSON retain only bounded metadata and SHA-256 fingerprints. The browser-built ZIP can include the IndexedDB file bytes alongside `certificate.pdf`, `manifest.json`, `report.html`, `hashes.sha256` and `verification.json`. The certificate has a stable record-bound token, a visual Aright platform seal and a holder/organization field; GitHub code imports prefill it from the public repository-owner namespace, while other media require the user to enter a name or organization before download. It records the analysis signal and platform issuance but does not establish identity, authorship or ownership. Adding or removing a file never checks or unchecks the task. Hosted evidence also records the runtime credential slot, attempt count, failover flag, and normalized reason code; these are execution roles, never key material, prefixes, hashes, or provider credential bodies. See `THIRD_PARTY_NOTICES.md` for exact licenses, revisions, hashes, and the audio-model licensing caveat.
...
Reports and workflow state are stored in the browser's `localStorage`; optional supporting-file bytes are stored separately in IndexedDB and are not embedded in JSON exports. The prominent Plan action opens a printable full report that can be saved as PDF through the browser; collapsed on-screen Aright plan details and the editable Human-work statement are expanded for print. The evidence-package download is a local ZIP containing `certificate.pdf`, a manifest, a human-readable report with the full Aright plan, attachment hashes, verification metadata and any locally stored supporting files. The certificate PDF and every included attachment are listed in `hashes.sha256`. Its package fingerprint helps detect accidental changes; it is not a trusted timestamp, signer identity, digital signature or tamper-evident evidence vault. Browser storage remains unencrypted, per-browser and user-editable.
...
- `tests/cloud-api.test.mjs`, `tests/github-code.test.mjs`, `tests/openai-copilot.test.mjs`, `tests/report-export.test.mjs`: mocked provider, credential-failover, GitHub-import, Aright-plan/OpenAI-adapter, and report-package contract tests
