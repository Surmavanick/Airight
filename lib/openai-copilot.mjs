...
const planItemSchema = {
  ...
  properties: {
    ...
    steps: { type: "array", maxItems: 8, items: { type: "string", maxLength: 500 } },
    acceptanceCriteria: { type: "array", maxItems: 8, items: { type: "string", maxLength: 500 } },
    evidenceToCollect: { type: "array", maxItems: 8, items: { type: "string", maxLength: 500 } },
  },
};
...
const COPILOT_INSTRUCTIONS = `You are Aright Evidence Copilot. You improve review plans and evidence documentation; you do not detect or prove authorship.
...
An AI-detector percentage is an immutable screening signal, not an authorship percentage or proof. A checked task is a user self-attestation, not verified completion. Attachment metadata and SHA-256 hashes show that files were selected; they do not verify file contents or claims.
For generate_plan, return one practical, asset-specific plan item per supplied task id. Make steps and acceptance criteria concrete. Draft an editable Human-work statement that clearly separates known record facts from work the user still needs to describe or evidence. Put unknowns in missingEvidence.
...
Use concise professional English. The disclaimer must say the output is an AI-generated review aid and not proof or legal advice.`;
...
export function getOpenAiCopilotConfig(env = process.env) {
  ...
  return {
    ...
    timeoutMs: Number.isFinite(timeout) ? Math.floor(clamp(timeout, 5_000, 120_000)) : 45_000,
...
function normalizePlanOutput(value, input) {
  ...
  const seen = new Set();
  const detailedPlan = (Array.isArray(value?.detailedPlan) ? value.detailedPlan : []).map((item) => {
    const taskId = safeIdentifier(item?.taskId);
    if (!taskIds.has(taskId) || seen.has(taskId)) return null;
    seen.add(taskId);
    return {
      taskId,
  ...
  if (input.record.tasks.length && !detailedPlan.length) {
    throw new CopilotError("OpenAI returned a plan that did not match the review tasks.", 502, "OPENAI_INVALID_RESPONSE");
  }
...
export async function runOpenAiCopilot(inputValue, options = {}) {
  ...
  const timeoutMs = Number.isFinite(timeout) ? Math.floor(clamp(timeout, 5_000, 120_000)) : 45_000;
  ...
  const requestBody = {
    ...
    max_output_tokens: input.action === "generate_plan" ? 4_000 : 1_500,
  };
