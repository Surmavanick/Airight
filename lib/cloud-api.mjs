...
import { GithubImportError, importGithubRepository } from "./github-code.mjs";
import { handleOpenAiCopilot, openAiCopilotStatus } from "./openai-copilot.mjs";
...
const DETECT_KINDS = new Set(["text", "file", "image", "audio", "github", "copilot"]);
const POLICY = "Provider-only screening: <=25% likely human, >=75% likely AI, otherwise inconclusive";
const FAILOVER_HTTP_STATUSES = new Set([401, 402, 403]);
const PROVIDER_TOTAL_BUDGET_MS = 165_000;
...
function getConfig(env = process.env) {
  ...
  const primaryApiKey = String(env.AIORNOT_API_KEY || "").trim();
  const backupCandidate = String(env.AIORNOT_API_KEY_BACKUP || "").trim();
  return {
    apiKey: primaryApiKey,
    backupApiKey: primaryApiKey && backupCandidate && backupCandidate !== primaryApiKey ? backupCandidate : "",
    githubToken: String(env.GITHUB_API_TOKEN || "").trim(),
...
function sanitizeProviderPayload(value, secrets, depth = 0) {
  ...
  const secretList = [...new Set((Array.isArray(secrets) ? secrets : [secrets]).filter(Boolean))]
...
async function callProviderAttempt(url, makeInit, config, fetchImpl, apiKey, timeoutMs = config.timeoutMs) {
  let response;
  const init = makeInit();
  ...
  if (text) {
    try {
      ...
      if (response.ok) throw new ApiError("AI or Not returned an invalid response.", 502, "INVALID_PROVIDER_RESPONSE");
    }
  }
  const payload = sanitizeProviderPayload(parsed, [config.apiKey, config.backupApiKey]);
  return { response, payload };
}

async function callProvider(url, makeInit, config, fetchImpl) {
  const startedAt = Date.now();
  const primary = await callProviderAttempt(url, makeInit, config, fetchImpl, config.apiKey);
  if (primary.response.ok) {
    return {
      payload: primary.payload,
      meta: { credentialRoute: "primary", attemptCount: 1, failoverUsed: false, failoverReason: "" },
    };
  }

  if (config.backupApiKey && FAILOVER_HTTP_STATUSES.has(primary.response.status)) {
    const failoverReason = providerHttpError(primary.response.status, primary.payload).code;
    const remainingMs = PROVIDER_TOTAL_BUDGET_MS - (Date.now() - startedAt);
    if (remainingMs < 5_000) throw providerHttpError(primary.response.status, primary.payload);
    const backup = await callProviderAttempt(
      url,
      makeInit,
      config,
      fetchImpl,
    ...
    throw providerHttpError(backup.response.status, backup.payload);
  }

  throw providerHttpError(primary.response.status, primary.payload);
}
...
function rawRecord(modality, payload, normalized, providerMeta = {}) {
  ...
  return {
    ...
    providerExecution: {
      credentialSlot: providerMeta.credentialRoute || "primary",
      attemptCount: providerMeta.attemptCount || 1,
      failoverUsed: Boolean(providerMeta.failoverUsed),
      failoverReason: providerMeta.failoverReason || "",
    },
    providerResponse,
  };
}
...
async function detectImage(request, bytes, config, fetchImpl) {
  ...
  const providerCall = await callProvider(url, () => {
    const form = new FormData();
    form.append("image", new Blob([bytes], { type: mime }), fileName);
    return { method: "POST", body: form };
  }, config, fetchImpl);
  const data = normalizeImageProvider(providerCall.payload);
  return { data, raw: rawRecord("image", providerCall.payload, data, providerCall.meta) };
}
...
async function detectText(text, config, fetchImpl) {
  ...
  const data = normalizeTextProvider(providerCall.payload, value);
  return { data, raw: rawRecord("text", providerCall.payload, data, providerCall.meta) };
}
...
async function detectAudio(request, bytes, config, fetchImpl) {
  ...
  const providerCall = await callProvider(VOICE_ENDPOINT, () => {
    const form = new FormData();
    form.append("file", new Blob([wav], { type: "audio/wav" }), `${originalName || "voice"}.wav`);
    return { method: "POST", body: form };
  }, config, fetchImpl);
  const data = normalizeVoiceProvider(providerCall.payload, { sampleCount, duration: sampleCount / AUDIO_SAMPLE_RATE });
  return { data, raw: rawRecord("voice", providerCall.payload, data, providerCall.meta) };
}

async function detectGithub(bytes, config, fetchImpl) {
  if (!bytes.length) throw new ApiError("Add a public GitHub repository URL.", 400, "EMPTY_GITHUB_URL");
  if (bytes.length > 2_048) throw new ApiError("The GitHub import request is too large.", 413, "GITHUB_REQUEST_TOO_LARGE");
  let repositoryUrl = "";
  try {
    repositoryUrl = String(JSON.parse(bytes.toString("utf8")).repositoryUrl || "");
  } catch {
    throw new ApiError("The GitHub import request must be valid JSON.", 400, "INVALID_JSON");
  }
  try {
    return await importGithubRepository(repositoryUrl, { fetchImpl, token: config.githubToken });
  } catch (error) {
    if (error instanceof GithubImportError) throw new ApiError(error.message, error.status, error.code);
    throw error;
  }
}
...
export async function handleStatus(request, options = {}) {
  ...
  try {
    ...
    const copilot = openAiCopilotStatus(options.env, Boolean(config.adminPassword));
    const models = {
      ...
      code: { id: "GitHub repository importer + deterministic demo", loaded: configured },
      copilot: { id: copilot.model, loaded: copilot.configured, role: copilot.role },
    };
    const body = {
      online: true,
      ready: configured,
      ...
      providers: {
        ...
        github: { primary: "GitHub REST API", configured, authenticated: Boolean(config.githubToken), publicOnly: true, remoteProcessing: true },
        openai: copilot,
      },
      capabilities: {
        text: true,
        ...
        documents: ["txt"],
        githubPublicRepositories: true,
        evidenceCopilot: copilot.configured,
      },
      limits: {
      ...
      providerFailover: {
        configured: Boolean(config.backupApiKey),
      },
      worker: {
        worker: configured,
...
export async function handleDetection(request, options = {}) {
  ...
  if (kind === "copilot") return handleOpenAiCopilot(request, options);
  try {
    requestOrigin(request);
    const config = getConfig(options.env);
    ...
    else if (kind === "github") result = await detectGithub(bytes, config, fetchImpl);
