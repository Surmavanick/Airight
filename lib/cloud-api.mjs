...
function providerHttpError(status) {
  if (status === 401 || status === 403) return new ApiError("AI or Not rejected the configured API key.", 502, "PROVIDER_AUTH_FAILED");
...
export async function handleStatus(request, options = {}) {
  ...
  try {
    ...
    const models = {
      ...
      audio: { id: "AI or Not v1 voice", loaded: configured },
    };
    const body = {
      online: true,
      ready: configured,
      ...
      providers: {
        ...
        audio: { primary: "AI or Not v1 voice", configured, scope: "speech only", remoteProcessing: true },
