...
const IMPORT_DEADLINE_MS = 145 * 1000;
...
async function githubJson(url, { fetchImpl, token, maxBytes = JSON_MAX_BYTES, accept, deadlineAt = Number.POSITIVE_INFINITY } = {}) {
  const remainingMs = Math.min(15_000, deadlineAt - Date.now());
  if (remainingMs <= 0) {
    throw new GithubImportError("GitHub import exceeded its safe processing time.", 504, "GITHUB_TIMEOUT");
  }
  let response;
  try {
...
async function importSampleFiles(entries, context) {
  ...
  for (const entry of entries) {
    try {
      ...
      const payload = await githubJson(url, { fetchImpl: context.fetchImpl, token: context.token, maxBytes: 180 * 1024, deadlineAt: context.deadlineAt });
      ...
      if (error instanceof GithubImportError && error.code === "GITHUB_RATE_LIMIT") {
        ...
        return { files, warnings: warnings.slice(0, 4), sampledBytes, sampledLines, rateLimited: true };
      }
      warnings.push(`Skipped ${entry.path} because its source sample was unavailable.`);
    }
  }
  return { files, warnings: warnings.slice(0, 4), sampledBytes, sampledLines, rateLimited: false };
}

async function importRepositoryEnrichment(context) {
  const warnings = [];
  let contributors = [];
  let contributorsHasMore = false;
  let latestCommit = null;

  if (context.deadlineAt - Date.now() < 5_000) {
    return { contributors, contributorsHasMore, latestCommit, warnings: ["Repository activity enrichment was skipped to keep the import within its processing deadline."] };
  }

  try {
    const endpoint = `${GITHUB_API}/repos/${encodeURIComponent(context.owner)}/${encodeURIComponent(context.repo)}/contributors?per_page=13&anon=0`;
    const payload = await githubJson(endpoint, { fetchImpl: context.fetchImpl, token: context.token, maxBytes: 512 * 1024, deadlineAt: context.deadlineAt });
    if (Array.isArray(payload)) {
      const valid = payload.map((item) => {
        const login = cleanLogin(item?.login);
        if (!login) return null;
        return {
          login,
          url: safeGithubUrl(item?.html_url) || `https://github.com/${encodeURIComponent(login)}`,
          type: cleanText(item?.type, 30) || "User",
          contributions: Math.max(0, Math.floor(Number(item?.contributions) || 0)),
        };
      }).filter(Boolean);
      contributorsHasMore = valid.length > 12 || payload.length > 12;
      contributors = valid.slice(0, 12);
    }
  } catch (error) {
    warnings.push(error instanceof GithubImportError && error.code === "GITHUB_RATE_LIMIT"
      ? "GitHub rate limiting prevented contributor enrichment."
      : "Top-contributor metadata was unavailable for this import.");
    if (error instanceof GithubImportError && error.code === "GITHUB_RATE_LIMIT") {
      return { contributors, contributorsHasMore, latestCommit, warnings };
    }
  }

  if (context.deadlineAt - Date.now() < 5_000) {
    warnings.push("Latest-commit enrichment was skipped to keep the import within its processing deadline.");
    return { contributors, contributorsHasMore, latestCommit, warnings };
  }

  try {
    const endpoint = `${GITHUB_API}/repos/${encodeURIComponent(context.owner)}/${encodeURIComponent(context.repo)}/commits?sha=${encodeURIComponent(context.defaultBranch)}&per_page=1`;
    const payload = await githubJson(endpoint, { fetchImpl: context.fetchImpl, token: context.token, maxBytes: 512 * 1024, deadlineAt: context.deadlineAt });
    const item = Array.isArray(payload) ? payload[0] : null;
    const sha = cleanText(item?.sha, 64);
    if (item && sha) {
      const login = cleanLogin(item?.author?.login);
      const name = cleanText(item?.commit?.author?.name, 100);
      latestCommit = {
        sha,
        url: safeGithubUrl(item?.html_url),
        message: cleanText(String(item?.commit?.message || "").split(/\r?\n/)[0], 160),
        date: cleanDate(item?.commit?.author?.date || item?.commit?.committer?.date),
        verified: Boolean(item?.commit?.verification?.verified),
        author: {
          login,
          name: name || login || "Unknown author",
          url: login ? safeGithubUrl(item?.author?.html_url) || `https://github.com/${encodeURIComponent(login)}` : "",
        },
      };
    }
  } catch (error) {
    warnings.push(error instanceof GithubImportError && error.code === "GITHUB_RATE_LIMIT"
      ? "GitHub rate limiting prevented latest-commit enrichment."
      : "Latest default-branch commit metadata was unavailable for this import.");
  }
...
export async function importGithubRepository(repositoryUrl, options = {}) {
  ...
  const deadlineAt = Number.isFinite(options.deadlineAt) ? Number(options.deadlineAt) : Date.now() + IMPORT_DEADLINE_MS;
  ...
  const metadata = await githubJson(repoEndpoint, { fetchImpl, token, deadlineAt });
  ...
  const tree = await githubJson(treeEndpoint, { fetchImpl, token, maxBytes: TREE_MAX_BYTES, deadlineAt });
  ...
  if (!sample.files.length) {
    if (sample.rateLimited) {
      throw new GithubImportError("GitHub's public API rate limit stopped the source sample. Try again after the reset time or configure GITHUB_API_TOKEN on the server.", 429, "GITHUB_RATE_LIMIT");
    }
  ...
  const enrichment = sample.rateLimited
    ? { contributors: [], contributorsHasMore: false, latestCommit: null, warnings: ["Repository activity enrichment was skipped because GitHub rate limiting stopped the source sample."] }
    : await importRepositoryEnrichment({
        owner: canonicalOwner,
        repo: canonicalRepo,
  ...
  const result = {
    data: {
      ...
      repository: {
        ...
        url: canonicalUrl,
        owner: {
          login: cleanLogin(metadata.owner?.login) || canonicalOwner,
          url: safeGithubUrl(metadata.owner?.html_url) || `https://github.com/${encodeURIComponent(canonicalOwner)}`,
          type: cleanText(metadata.owner?.type, 30) || "User",
        },
        description: cleanText(metadata.description, 500),
        homepage: safeHomepage(metadata.homepage),
        primaryLanguage: cleanText(metadata.language, 80),
        topics: Array.isArray(metadata.topics) ? metadata.topics.map((topic) => cleanText(topic, 50)).filter(Boolean).slice(0, 12) : [],
        createdAt: cleanDate(metadata.created_at),
        updatedAt: cleanDate(metadata.updated_at),
        pushedAt: cleanDate(metadata.pushed_at),
        stats: {
          stars: Math.max(0, Math.floor(Number(metadata.stargazers_count) || 0)),
          forks: Math.max(0, Math.floor(Number(metadata.forks_count) || 0)),
          openIssuesAndPulls: Math.max(0, Math.floor(Number(metadata.open_issues_count) || 0)),
          githubSizeKb: Math.max(0, Math.floor(Number(metadata.size) || 0)),
        },
        defaultBranch: String(metadata.default_branch),
        ...
        isTemplate: Boolean(metadata.is_template),
        disabled: Boolean(metadata.disabled),
        forkParent: metadata.parent?.full_name ? {
          fullName: cleanText(metadata.parent.full_name, 150),
          url: safeGithubUrl(metadata.parent.html_url),
        } : null,
        ...
        contributorsHasMore: enrichment.contributorsHasMore,
        latestCommit: enrichment.latestCommit,
        importedAt,
        warnings: [...sample.warnings, ...enrichment.warnings].slice(0, 8),
      },
      model: {
    ...
    raw: {
      ...
      coverage: {
        ...
        sampledBytes: sample.sampledBytes,
        treeTruncated: Boolean(tree.truncated),
        contributorProfiles: enrichment.contributors.length,
        latestCommitAvailable: Boolean(enrichment.latestCommit),
      },
      method: {
        id: "aright/github-vibe-estimator",
